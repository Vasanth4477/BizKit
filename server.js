...
app.post("/api/auth/signup", async (req, res) => {
  ...
  try {
    ...
    db.prepare(
      "INSERT INTO business_profiles(user_id,business_name,email) VALUES(?,?,?)"
    ).run(info.lastInsertRowid, name.trim(), normalized);
    const user = {
...
app.get("/api/customers", auth, (req, res) =>
  res.json({
    customers: db
      .prepare(
        "SELECT id,name,phone,email FROM customers WHERE user_id=? ORDER BY id DESC"
      )
      .all(req.user.id),
  })
);
app.post("/api/customers", auth, (req, res) => {
  ...
  const info = db
    .prepare("INSERT INTO customers(user_id,name,phone,email) VALUES(?,?,?,?)")
    .run(req.user.id, name, phone, email);
  res
    .status(201)
    .json({ customer: { id: info.lastInsertRowid, name, phone, email } });
});
app.delete("/api/customers/:id", auth, (req, res) => {
  db.prepare("DELETE FROM customers WHERE id=? AND user_id=?").run(
    req.params.id,
    req.user.id
  );
  res.json({ ok: true });
...
app.get("/api/products", auth, (req, res) =>
  res.json({
    products: db
      .prepare(
        "SELECT id,name,price,sku FROM products WHERE user_id=? ORDER BY id DESC"
      )
      .all(req.user.id),
  })
);
app.post("/api/products", auth, (req, res) => {
  ...
  if (!name)
    return res.status(400).json({ error: "Product/service name is required" });
  const info = db
    .prepare("INSERT INTO products(user_id,name,price,sku) VALUES(?,?,?,?)")
    .run(req.user.id, name, Number(price) || 0, sku);
  res
    .status(201)
    .json({
      product: {
...
app.get("/api/profile", auth, (req, res) => {
  const profile =
    db
      .prepare(
        "SELECT business_name as businessName,phone,email,gstin,address FROM business_profiles WHERE user_id=?"
      )
      .get(req.user.id) || {};
  res.json({ profile });
});
app.put("/api/profile", auth, (req, res) => {
  ...
  db.prepare(
    `INSERT INTO business_profiles(user_id,business_name,phone,email,gstin,address)
    VALUES(?,?,?,?,?,?)
...
app.get("/api/quotations", auth, (req, res) =>
  res.json({
    quotations: db
      .prepare("SELECT * FROM quotations WHERE user_id=? ORDER BY id DESC")
      .all(req.user.id)
...
app.post("/api/quotations", auth, (req, res) => {
  const q = req.body || {},
    ...
    i = db
      .prepare(
        "INSERT INTO quotations(user_id,quote_no,quote_date,valid_until,customer_name,customer_address,gst_rate,discount,items_json,subtotal,gst_amount,total,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)"
      )
      .run(
        req.user.id,
        q.quoteNo || "QUO-001",
...
app.delete("/api/quotations/:id", auth, (req, res) => {
  db.prepare("DELETE FROM quotations WHERE id=? AND user_id=?").run(
    req.params.id,
    req.user.id
  );
  res.json({ ok: true });
...
app.post("/api/invoices", auth, (req, res) => {
  const q = req.body || {},
    ...
    i = db
      .prepare(
        "INSERT INTO invoices(user_id,invoice_no,invoice_date,due_date,customer_name,customer_address,gst_rate,discount,items_json,subtotal,gst_amount,total,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)"
      )
      .run(
        req.user.id,
        q.invoiceNo || "INV-001",
...
app.patch("/api/invoices/:id/status", auth, (req, res) => {
  const status = ["unpaid", "paid", "overdue", "cancelled"].includes(
    req.body?.status
  )
    ? req.body.status
    : "unpaid";
...
app.delete("/api/invoices/:id", auth, (req, res) => {
  db.prepare("DELETE FROM invoices WHERE id=? AND user_id=?").run(
    req.params.id,
    req.user.id
  );
  res.json({ ok: true });
...
app.get("/api/dashboard", auth, (req, res) => {
  const inv = db
      .prepare("SELECT status,total FROM invoices WHERE user_id=?")
      .all(req.user.id),
    customers = db
      .prepare("SELECT COUNT(*) c FROM customers WHERE user_id=?")
      .get(req.user.id).c,
    products = db
      .prepare("SELECT COUNT(*) c FROM products WHERE user_id=?")
      .get(req.user.id).c;
  res.json({
    customers,
    products,
...
app.get("/api/health", (req, res) =>
  res.json({ ok: true, service: "BizKit API" })
);
...
app.listen(PORT, () =>
  console.log(`BizKit running on http://localhost:${PORT}`)
);