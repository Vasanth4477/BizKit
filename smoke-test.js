...
const requiredServer=['DATABASE_URL','pg','/api/integrations/razorpay/payment-link','CREATE TABLE IF NOT EXISTS payments','ALTER TABLE payments ADD COLUMN IF NOT EXISTS reversed_at','CREATE TABLE IF NOT EXISTS stock_movements','/api/health','const PAGE_FILES','withTransaction','Insufficient stock','Payment exceeds the remaining invoice balance','/api/payments/:id/reverse','/api/purchases/:id','/api/purchases/:id/cancel','/api/suppliers/:id','convert-to-invoice'];
...
for(const x of ['customerOptions','purchaseProductOptions','invoiceBuilder','purchaseModalTitle','supplierModalTitle','purchaseSaveBtn','supplierSaveBtn']) if(!router.includes(x)) throw Error('Missing workflow UI: '+x);
